...
void LevelModel::loadLevel(int levelId) {
    ...
    m_levelId = levelId; // On m�morise le niveau
    m_mapData.clear();

    // --- NIVEAU 1 : LA FOR�T (Ton code actuel) ---
    if (levelId == 1) {
        m_tileSize = 24.0f;
    ...
    for (int i = gridY - 3; i <= gridY + 3; i++) {
        for (int j = gridX - 3; j <= gridX + 3; j++) {
            if (i >= 0 && i < m_mapData.size() && j >= 0 && j < m_mapData[i].size()) {
                ...
                // SECURITE : On ne consid�re comme MUR que les chiffres 1, 2, 3, 4
