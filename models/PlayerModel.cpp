...
void PlayerModel::attack() {
    if (state != PlayerState::ATTACK && state != PlayerState::DEAD && !isDashing) {
        ...
        attackTimer = 0.0f;
        m_velocity.x = 0; // On s'arr�te pour frapper
        m_hasDealtDamage = false;
    }
}
...
void PlayerModel::dash() {
    if (dashCooldownTimer <= 0 && !isDashing && state != PlayerState::DEAD) {
        isDashing = true;
        dashDurationTimer = 0.0f;
        dashCooldownTimer = 0.5f; // D�lai avant le prochain dash
...
void PlayerModel::update(float deltaTime) {
    ...
    if (isDashing) {
        ...
        m_velocity.y = 0; // Pas de gravit� pendant le dash
        dashDurationTimer += deltaTime;

        if (dashDurationTimer >= 0.2f) { // Dur�e du dash
            isDashing = false;
            m_velocity.x = 0;
    ...
    // Application du mouvement � la position h�rit�e d'Entity
    m_position += m_velocity;

    // D�termination de l'�tat JUMP/FALL si on n'est pas en train d'attaquer
    ...
    // Gestion du timer d'attaque pour revenir � l'�tat IDLE
    if (state == PlayerState::ATTACK) {
        attackTimer += deltaTime;
        if (attackTimer >= 0.32f) { // Dur�e de l'animation d'attaque
...
sf::FloatRect PlayerModel::getHitbox() const {
    // Hitbox ajust�e pour un ninja de 30x60 pixels
    float width = 30.0f;
    float height = 60.0f;
